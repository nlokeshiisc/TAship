...
full_adder = RNN(input_size=2, hidden_size=params[constants.HIDSIZE], output_size=1).to(device)
adder_trn = BinaryAddData(dim=params[constants.NUM_BITS], num=params[constants.NUM_TRN], msb_first=False)
adder_tst = BinaryAddData(dim=params[constants.NUM_BITS], num=params[constants.NUM_TST], msb_first=False)
trn_loader = torch.utils.data.DataLoader(adder_trn, batch_size=params[constants.BSZ], shuffle=True)
tst_loader = torch.utils.data.DataLoader(adder_tst, batch_size=params[constants.NUM_TST], shuffle=False)
